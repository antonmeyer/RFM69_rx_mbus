...
boolean RFM69::initDevice(unsigned char PinNSS, unsigned char PinDIO0, rfm69type DeviceType, float Frequency, modulation Modulation, unsigned long BitRate, unsigned long fdev, int PreambleLength, int8_t TxPower) {
	...
	//set device type
	if (DeviceType == W || DeviceType == CW) _HighPowerDevice = false;
	else if (DeviceType == HW || DeviceType == HCW) _HighPowerDevice = true;
	else return(false);
	//check device
	delay(10);
	Type = readSPI(RFM69_REG_10_VERSION);
	if (Type == 00 || Type == 0xFF) return(false);
	//init device mode
	_Mode = INITIALIZING;
	setModeStdby();
	...
	//set packet mode variable length, whitening, CRC on, no address filtering
	writeSPI(RFM69_REG_37_PACKETCONFIG1, 0x80); //was 0xD0
	//set PreambleLength
...
boolean RFM69::waitForChannelFree(float RSSI_Threshold, unsigned long TimeoutFreeChannel) {
	...
	while (!checkIsChannelFree(RSSI_Threshold)) {
		//channel not free
		if ((long)(millis() - WaitUntilTime) > 0) return(false);
		delay(5);
	}
	return(true);
}
...
boolean RFM69::checkIsChannelFree(float RSSI_Threshold) {
	...
	while ((readSPI(RFM69_REG_23_RSSICONFIG) & RFM69_RSSICONFIG_RSSIDONE) == 0) {
		if ((long)(millis() - TimeoutTime) > 0) break;
	}
	RSSI = -(float)(readSPI(RFM69_REG_24_RSSIVALUE))/2.;
	if (RSSI > RSSI_Threshold) return(false);
	else return(true);
}
...
boolean RFM69::awaitSizedFrame(unsigned char Size, unsigned long Timeout) {
	...
	while ((long)(millis() - TimeoutTime) < 0) {
		if (receiveSizedFrame(Size)) return(true);
	}
	setModeStdby();
	return(false);
}
...
boolean RFM69::receiveSizedFrame(unsigned char Size) {
	...
	if (readSPI(RFM69_REG_28_IRQFLAGS2) & RFM69_IRQFLAGS2_FIFOLEVEL) {
		...
		readFifo(_RxBuffer, Size); // read the first block
...
unsigned char RFM69::rxMBusMsg() {
	...
	if (readSPI(RFM69_REG_28_IRQFLAGS2) & RFM69_IRQFLAGS2_FIFOLEVEL) {
		...
		if (!(this->msgerr) && (_msgLen >10) && (_msgLen < 100)) {
			...
			while (!(readSPI(RFM69_REG_28_IRQFLAGS2) & (RFM69_IRQFLAGS2_FIFOLEVEL | RFM69_IRQFLAGS2_FIFOOVERRUN) ) && (timeout > millis())) {}; //busy loop
...
void RFM69::setBitRate(unsigned long BitRate) {
	...
	if (BitRate < 1200) BitRate = 1200;
	if (BitRate > 300000) BitRate = 300000;
	if (_Modulation == OOK && BitRate > 19200) BitRate = 19200;
...
void RFM69::setSyncWords(unsigned char *SyncWords, unsigned char Len) {
	unsigned char SyncConfig;
	if (Len > 8) Len = 8;
	if (Len > 0) writeSPIBurst(RFM69_REG_2F_SYNCVALUE1, SyncWords, Len);
	SyncConfig = readSPI(RFM69_REG_2E_SYNCCONFIG);
	SyncConfig &= 0x7F;
...
void RFM69::useSyncWords(boolean Using) {
	unsigned char SyncConfig;
	SyncConfig = readSPI(RFM69_REG_2E_SYNCCONFIG);
	if (Using) SyncConfig |= 0x80;
	else SyncConfig &= 0x7F;
	writeSPI(RFM69_REG_2E_SYNCCONFIG, SyncConfig);
}


void RFM69::setOverCurrentProtection(boolean On) {
	if (On) writeSPI(RFM69_REG_13_OCP, RFM69_OCP_ON);
	else writeSPI(RFM69_REG_13_OCP, RFM69_OCP_OFF);
}
...
void RFM69::setModeRx() {
	if (_Mode != RX) {
		...
		memcpy (_RxBuffer,0,sizeof(_RxBuffer));
		memcpy (_mbusmsg,0,sizeof(_mbusmsg));
		writeSPI(RFM69_REG_3C_FIFOTHRESH,MSGBLK1); 
...
void RFM69::setOpMode(unsigned char Mode) {
	...
	while (!(readSPI(RFM69_REG_27_IRQFLAGS1) & RFM69_IRQFLAGS1_MODEREADY)); //wWait for mode to change.
}
...
unsigned char RFM69::decode3o6Block (unsigned char  *encoded, unsigned char *decoded , unsigned char encodedSize) {
	...
	while (bytesRemaining) {
		...
		if (bytesRemaining == 1)
		{
			...
			if (decodingStatus == DecErr) return (DecErr);
			bytesRemaining  -= 1;
			bytesEncoded    += 1;
		}
		else
